...
class Patiant_File_NO(models.Model):
    ...
    def __str__(self):
         return str(self.ar_patiant_name) + '  - ' + str(self.en_patiant_name)
...
class Patiant_Or_Appointment(models.Model):
    ...
    Operation_Name = models.ForeignKey(Operation_type , on_delete=models.CASCADE , related_name='Operation_Name', null=True, blank=True)
    ...
    def save(self, *args, **kwargs):
        ...
        number = str(self.Patiant_Phone)
        day = self.Date.day
        date = self.Date.strftime('%m-%d-%Y')
        time = self.begin_time.strftime('%H:%M')
        text = re.sub(r'[!"\$%&\'()*+,\-.\/:;=#@?\[\\\]^_`{|}~،ًُ]','',number)
        print(name)
        print(text)
        print(day)
        print(date)
        print(time)
        Send_SMS(text , name  , day, date , time)
        return super().save(*args, **kwargs)
    class Meta:
